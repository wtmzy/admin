<template>
  <vab-dialog v-model="drawerVisible" append-to-body title="标签设置" width="400px">
    <el-form ref="form" label-position="top" :model="theme">
      <el-form-item v-if="theme.showTabs" label="标签风格">
        <el-radio-group v-model="theme.tabsBarStyle">
          <el-radio-button v-for="item in tabsBarStyleList" :key="item.value" :label="item.label" :value="item.value" />
        </el-radio-group>
      </el-form-item>
      <el-form-item label="标签图标">
        <el-switch v-model="theme.showTabsIcon" />
      </el-form-item>
      <el-form-item label="持久化标签">
