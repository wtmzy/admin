<template>
  <el-drawer v-model="drawerVisible" append-to-body class="vab-drawer" direction="rtl" :size="size" title="主题配置">
    <el-scrollbar height="calc(var(--vh, 1vh) * 100 - 120px)">
      <el-form ref="form" label-position="left" :model="theme">
        <el-form-item v-if="device !== 'mobile' && routeName !== 'SeparateLayout'" class="vab-shop-layout-item" label="布局">
        ...
        <el-form-item label="主题">
          <el-radio-group v-model="theme.themeName" @change="_updateTheme">
            <el-radio-button v-for="item in themeNameList" :key="item.value" :label="item.label" :value="item.value" />
          </el-radio-group>
        </el-form-item>
        <el-form-item
        ...
        <el-form-item v-if="'technology' != theme.themeName" label="配色">
          <vab-color-picker />
        </el-form-item>
        <el-form-item v-if="'default' === theme.themeName && mode !== 'dark'" label="菜单背景跟随配色">
          <el-switch v-model="theme.isFollow" @change="updateIsFollow" />
        </el-form-item>
        <el-form-item v-if="theme.layout !== 'horizontal'" label="菜单宽度">
        ...
        <el-form-item v-if="theme.showTabs" label="持久化标签">
          <el-switch v-model="persistenceTab" @change="handlePersistenceTab" />
        </el-form-item>
        <el-form-item v-if="theme.showTabs" label="标签图标">
          <el-switch v-model="theme.showTabsIcon" />
        </el-form-item>
        <el-form-item v-if="theme.showTabs" label="标签风格">
          <el-select v-model="theme.tabsBarStyle">
            <el-option v-for="item in tabsBarStyleList" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="device !== 'mobile' && theme.showTabs" label="标签拖拽">
          <el-switch v-model="theme.tabDrag" />
        </el-form-item>
        <el-form-item label="页脚">
          <el-switch v-model="theme.showFooter" @change="handleShowFooter" />
        </el-form-item>
        <el-form-item v-if="theme.layout === 'column'" label="分栏风格">
          <el-select v-model="theme.columnStyle">
            <el-option v-for="item in columnStyleList" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="圆角">
          <el-input-number v-model="theme.radius" :max="26" :min="3" @change="handleRadius" />
        </el-form-item>
        <el-form-item v-if="'technology' != theme.themeName && 'plain' != theme.themeName" label="色弱">
        ...
        <el-form-item v-if="'technology' != theme.themeName && 'plain' != theme.themeName" label="暗黑组件">
        ...
        <el-form-item label="右侧浮窗">
          <el-switch v-model="theme.showThemeSetting" />
        </el-form-item>
        <el-form-item label="页面动画">
          <el-select v-model="theme.pageTransition">
            <el-option v-for="item in pageTransitionList" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
      </el-form>
    </el-scrollbar>
